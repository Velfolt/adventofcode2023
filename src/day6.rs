...
use nom::{
    ...
    sequence::tuple,
    IResult, Parser, character::complete::{alpha1, digit1},
};
use rayon::prelude::*;
...
pub fn day6_1() {
    ...
    let code: usize = time.iter().zip(distance.iter()).map(|(time, distance)| {
        (0..*time).filter(|hold| hold * (time - hold) > *distance).count()
    }).inspect(|x| println!("{:?}", x)).product();
...
fn boat_race_ignore_space(input: &str) -> IResult<&str, (Vec<usize>, Vec<usize>)> {
    ...
    let time: usize = time.iter().map(|x| x.to_string()).collect::<String>().parse().unwrap();
    
    let (input, _) = space(input)?;
    
    let (input, _) = tag("Distance: ")(input)?;
    let (input, distance) = separated_list1(many1(tag(" ")), digit_as_str)(input)?;
    let distance: usize = distance.iter().map(|x| x.to_string()).collect::<String>().parse().unwrap();
    let (input, _) = space(input)?;
